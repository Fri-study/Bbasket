...
function AddBookMark() {
  ...
  const SearchURLHandler = async () => {
    ...
    try {
      ...
      //태그는 해시태그를 추가한다
      let hashtaggedTags = scrapedTags
        .map((tag) => {
          if (tag.startsWith("#")) {
            return tag;
          } else {
  ...
  return (
    <form>
      <Navbar />
      <h4>
        개발용 프록시 사용을 위해 아래 링크에서 버튼을 눌러주세요(하루에
        한번정도만 누르면 OK)
      </h4>
      <a
        className="bg-slate-200 w-[200px] h-[30px] border-4 border-cyan-900"
      ...
      <div className="w-full flex flex-wrap justify-centert text-center justify-center mt-[20px]">
        <div className=" w-[600px] h-96 bg-blue-400 m-1 p-2 rounded-md flex flex-wrap">
          <div id="addBookMarkTopDiv" className="w-[600px]">
            <input
              id="addBookMarkURL"
              value={inputURLvalue}
              onChange={urlValueChangeHeandler}
              onKeyDown={urlKeyDownhandle}
              className="w-[500px] h-9 mANDp1 rounded-lg border-4"
              type="text"
              placeholder="북마크할 URL주소를 입력하세요"
            ></input>
            <button
              id="searchURLbtn"
              onClick={SearchURLHandler}
              className="text-xs s_btn text-cyan-50"
              type="button"
            >
              URL찾기
            </button>
          ...
          <div id="addBookMarkLeftDiv">
            <div className="place-content-start items-start flex w-[600px] h-36">
              <div
                id="theThumbnailDiv"
                className="inline-block w-48 m-1 mt-3 bg-white rounded-md h-28"
              >
                {okRespons && (
                  <img
                    id="theThumbnailImg"
                    className="w-48 rounded-md h-28"
                    src={okRespons ? data.get("scrapedImg") || "" : ""}
                    alt=""
                  />
                )}
              </div>
              <div className="inline-block h-24 text-left w-96">
                <input
                  id="theTitle"
                  className="inline border-4 roundOneInput mANDp1"
                  placeholder="원문제목"
                  defaultValue={okRespons ? data.get("scrapedTitle") || "" : ""}
                />
                <input
                  id="theAuthor"
                  className="inline border-4 roundOneInput mANDp1"
                  placeholder="원문출처"
                  defaultValue={
                    okRespons ? data.get("scrapedAuthor") || "" : ""
                  }
                />
                <textarea
                  id="theTags"
                  className="w-[370px] h-12 border-4 mANDp1 rounded-lg"
                  type="textarea"
                  placeholder="#태그"
            ...
            <div className="block">
              <textarea
                type="textarea"
                id="newMemo"
                className="w-[570px] h-[130px] border-4 mANDp1 rounded-lg block"
                placeholder="북마크에대한 메모를 남겨주세요"
              />
            </div>
          </div>
        </div>
        ...
        <div
          id="addBookMarkRightDiv"
          className="w-[200px] h-[200px] m-1 p-2 rounded-md bg-slate-100"
        >
          <p className="mANDp1">폴더선택</p>
          <select
            id="userFolderSelect"
            className="h-[36px] w-[120px] border-4 mANDp1 rounded-lg"
          >
            {!isEmpty(optionData) &&
...
async function Iframescraping(isIframe, alreadySetProxy) {
  ...
  try {
    ...
    let scrapedAuthor = $iframeData('[property="og:site_name"]').attr(
      "content"
    );
    let scrapedImg = $iframeData("img").attr("src");

    if (isEmpty(scrapedAuthor) && alreadySetProxy.includes("naver")) {
      scrapedAuthor = $iframeData('[property="naverblog:nickname"]').attr(
        "content"
      );
    }
    //todo 네이버태그를 구분해서?
