...
function Main(props) {
  ...
  const testVal = [
    {
      backdrop_path: "TEST THUMNAIL.png",
      title: "테스트북마크에대한제목1",
      from: "테스트출처1",
    },
    {
      backdrop_path: "TEST THUMNAIL-2.png",
      title: "테스트북마크에대한제목2",
      from: "테스트출처2",
    },
    {
      backdrop_path: "TEST THUMNAIL-3.png",
      title: "테스트북마크에대한제목3",
      from: "테스트출처3",
    },
    {
  ...
  const [state, setState] = useState({
    isMain: props.isMain || defaultMain,
    isShow: "beforeSearch",
    isValue: "",
  });
  ...
  const handleSearching = (searchTerm) => {
    forSearching(searchTerm, () => {
      state.isMain === "main" && changtoSearch(searchTerm);
    });
  };
    ...
    <div>
      ...
      {state.isMain === "main" && (
        <div>
          ...
          <button
            onClick={testModal}
            className="bg-slate-200 w-[220px] h-[30px] border-4 border-cyan-900"
          >
            {" "}
            북마크 등록 페이지 테스트 버튼
          </button>
      ...
      {/* 검색화면 진입시 */}
      {state.isMain === "searchmain" && (
        <SearchMain
          isMain={state.isMain}
          isShow={state.isShow}
          results={testVal}
          onSerchActed={handleSearching}
          isValue={state.isValue}
        />
      )}
    </div>
  );
